...
ping -n 2 127.0.0.1>nul
"%mainfolder%\alpha_mariadb\bin\mariadb.exe" --user=root --password=pwd --port=3306 -e "SELECT `proxy_address` INTO OUTFILE '../server_address.txt' FROM `alpha_realm`.`realmlist` WHERE `realm_id`=1";
